...
count = 1;
mul = [ 1, 1, -1]; add = [ 200, 200, 200 ]; scale = [100,500,1000];
...
for i=1:3
    surfs(:,:,i) = c(1).*X.^4 + c(2).*X.^3.*Y + c(3).*X.^2.*Y.^2 + ...
         ...
         mul(i).*c(13).*(add(i)+X) + mul(i).*c(14).*(add(i)+Y);
...
WriteImageSeries(Im, '/home/bollocks/sim/', 'ShSim', 'tif');
WriteImageSeries(surfs, '/home/bollocks/sim/', 'ShSurf', 'tif');
WriteImageSeries(CountIm, '/home/bollocks/sim/', 'Counts', 'tif');