...
databaseFile = "C:/Farsight_0.4.1/bin/NE.s3db"
images_path  = "C:/ccRCC_Pfizer_Samples/pERK"
...
for root, dirs, files in os.walk(images_path):
        for filename in fnmatch.filter(files, tableFiles):
                inpf  = os.path.join(root,filename)
                inpff = inpf.translate(trantab)
                inputFiles.append(inpff)
for root, dirs, files in os.walk(images_path):
        for filename in fnmatch.filter(files, inputImage):
                inpf  = os.path.join(root,filename)
                inpff = inpf.translate(trantab)
                inputImages.append(inpff)
